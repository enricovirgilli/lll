...
class PSD_cartesian(matrix2D):
    ...
    def pointtopixel(self, v):
        ...
        x=.5*self.pixels[0]+vpixel[0]
        y=.5*self.pixels[1]-vpixel[1] # A sign is changing...
        return [x,y]

    def xtics(self):
        return (pylab.arange(self.pixels[0])+0.5)*self.pitches[0]-self.sides[0]/2.

    def ytics(self):
        return (pylab.arange(self.pixels[1])+0.5)*self.pitches[1]-self.sides[1]/2.

    def measure(self, photon):
        '''Measures a photon'''
        pixel=self.pointtopixel(photon.r[:2])
...
class PSD(object):
    ...
    def measure(self, photon):
        self.cartesian.measure(photon)
        self.polar.measure(photon)
