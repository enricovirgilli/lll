...
class PhoXtal(object):
    ...
    def generator(self, Emin=1., Emax=1000., Exp=False,  sphi=0., stheta=pi):
        ...
        #stheta = pi - self.divergence_stheta(l-self.xtal.rho)
        stheta = pi - self.divergence_stheta(l)
    ...
    def random_position_on_surface(self):
        ...
        #random_pos_on_surface=[0,-0.5]
        random_pos_on_surface=[uniform(-x/2, x/2.) for x in self.xtal.dim[:2]]
        #random_pos_on_surface=[uniform(-x/2, 0) for x in self.xtal.dim[:2]]
    ...
    def local_eranges(self, Emin=1., Emax=1000., sphi=0., stheta=pi):
    ...
    def generator_fast(self, Emin=1., Emax=1000., Exp=0.,  sphi=0., stheta=pi):
        ...
        for i, cumweight in enumerate(cumweights):
            if rnd<=cumweight:
                self.generator(Emin=eranges[i][0], Emax=eranges[i][1], Exp=Exp, sphi=sphi, stheta=stheta)
    ...
    def fast_diff_bent(self, Focal, Emin=1., Emax=1000., Exp=0.,  sphi=0., stheta=pi): 
        #print "nominal angle", math.atan(self.xtal.rho/Focal)/math.pi*180
        l = math.sqrt(self.photon.r[0]**2 + self.photon.r[1]**2)
        ...
        if self.xtal.structure == "mosaic":
            for sigma in self.sigmas("gaussian"):
                ...
                if sigma > sigma_:
                    order = int(i)+1
                    sigma_ = sigma
            eranges = self.local_eranges(Emin=Emin, Emax=Emax, sphi=sphi, stheta=stheta)
            totaleranges =  self.diffracted_eranges(Emin=Emin, Emax=Emax, sphi=sphi, stheta=stheta)
            #print "e", eranges[0][0], eranges[0][1]
