...
EB = 100.0   # peak energy of diffraction
...
def peak_refl_curved(Z, keVB, keV, t= 0):
    tB=keV2Bragg(keVB)
...
def tbest(keVB, keV):
    ...
    #omega = (fwhm / 60) * (math.pi / 180) # conversion from arcmin to radians
    #omega = omega_s(keV)
    omega_arcsec = 30. #in arcsec
    omega = omega_arcsec * math.pi / (180.* 3600.)#in radiance
    muT = mu(Z,keV)
    tB=keV2Bragg(keVB)
    mu_angstrom = muT/1.0E8
    factor = ((math.pi**2) * d_hkl * math.cos(tB)) / (lambda0(keVB,keV)**2 * mu_angstrom * omega) 
    logarithm = log ( 1 + factor)
    return ((omega * logarithm)/((math.pi**2) * d_hkl / lambda0(keVB,keV)**2))/1.0E8 #tBest in cm
...
#####################################################################################
#For Cp vs Peak reflectivity.
#####################################################################################

def critical_values(keVB,keV):
    beta_critical = math.pi/(2*lambda0(keVB,keV))
    omega_arcsec = 30. #in arcsec
    omega = omega_arcsec * math.pi / (180.* 3600.)#in radiance
    delta_arcsec = 20.# in arc seconds.
    delta = delta_arcsec * math.pi / (180.* 3600.)#in radiance
    #######################################
    #calcuating Thickness for the critical beta.
    #--------------------------------------
    T_c = omega/(beta_critical*(delta/2)) #in Angstrom
    Th_c = T_c/1.0E8 #in centimeter
    
    #######################################
    #calcuating Curvature for the critical beta.
    #--------------------------------------
    C_p = beta_critical*(delta/2) # in 1/angstrom
    Cu_p = C_p * 1.0E10 #in 1/meter
    #######################################
    bestthick = omega/ Cu_p
    
    print "\nCritical Curvature =", Cu_p, "\nCritical Thickness = ", Th_c, "\nBest Thickness = ", bestthick, "\n"
    

def peak_refl_curved_vs_Cp(Z, keVB, keV, c_p):
   
    tB=keV2Bragg(keVB)
    #omega = 2 * math.atan( (t/2) / (internal_radius * 1000)) # Internal Radius
    omega_arcsec = 30. #in arcsec
    omega = omega_arcsec * 3.14 / (180.* 3600.)#in radiance
        
    cp = c_p#*10000000000. #in angstrom
    T = omega/cp*1.0E-10#in angstrom
    #cp = omega/T_c
    ...
    #muT = mu(Z,keV)
    #mu_angstrom = muT / 100000000.0
    diffraction = ( 1 - ( Numeric.exp(-(math.pi**2 * d_hkl) / (cp*1.0E-10 * lambda0(keVB,keV)**2) )) )
    absorption = Numeric.exp(- (mu(Z,keV)/1.0E10) * T / cos(tB))
    return diffraction * absorption

Curv_list = arange(0.001, 0.8, 0.002) #in 1/m 
aCurveP = []
eVB = 100.
for i, c in enumerate(Curv_list):
    aCurveP.append(peak_refl_curved_vs_Cp(Z, eVB, eVB, c))
      
critical_values(eVB,eVB)
print  tbest(eVB,eVB)

plot (Curv_list, aCurveP)
plt.axis([0, 0.8, 0, 1])
#plt.hold(True)
#plt.axis([0, 20, 0, 80])
plt.xlabel('Curvature in 1/m')
plt.ylabel('Peak Rflectivity')
plt.title('Peak Rflectivity vs Curvature')
plt.grid(True)
plt.savefig('PR_vs_curvature_Si111_100keV.ps')
plt.show()
