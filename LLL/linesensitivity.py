...
def sensitivity_at_keV(keV, fraction=0.5, Tobs=1.e5, nsigma=3., effdet=0.9, BG="default"):
    ...
    DeltaE= 1.0  #0.5 * keV + 2.05641071127e-15
    BG = Booklet.bg(keV)/2 
...
def linesensitivity(keV, fraction=0.5, Tobs=1.e5, nsigma=3., effdet=0.9, BG="default"):    
    radius = 0.1
    Ad=math.pi*radius**2
    BG = Booklet.bg(keV)
    deltaE = 40*keV/100
    this = ( 2 * BG * Ad + 0.001*crab(keV) * effdet * radius * (area(keV) + 0.0001) ) * deltaE
    num = nsigma * 1.31 * math.sqrt(this)
    den = effdet * radius * (area(keV) + 0.0001) * math.sqrt(Tobs)
    return num/den


enelist=arange(1, eup, 1)
for i, e in enumerate(enelist):
    #print e, sensitivity_at_keV(e, fraction=0.5, Tobs=1.e6, nsigma=3., effdet=1., BG="default")
    print e, linesensitivity(e, fraction=0.5, Tobs=1.e5, nsigma=3., effdet=0.9, BG="default")
