...
class Xtal(Material):
    ...
    def Erange(order=1, OFFSET=0., NUM_OF_SIGMA=4):
        """energy range diffracted by the crystal at a given order"""
        Dthetamax = NUM_OF_SIGMA*eta
        tB = self.gettB(OFFSET)
        Emin = Bragg2keV(tB + Dthetamax, order)
        Emax = Bragg2keV(tB - Dthetamax, order)
        return Emin, Emax


    print Erange(order=1, OFFSET=0., NUM_OF_SIGMA=4)
